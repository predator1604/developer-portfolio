...
public sealed class GetVisibleProjectsHandler(IProjectRepository repo)
    : IRequestHandler<GetVisibleProjectsQuery, Result<List<ProjectDto>>>
{
    public async Task<Result<List<ProjectDto>>> Handle(
        GetVisibleProjectsQuery _, CancellationToken ct)
    {
...
public sealed record GetProjectBySlugQuery(string Slug)
    : IRequest<Result<ProjectDto>>;

public sealed class GetProjectBySlugHandler(IProjectRepository repo)
    : IRequestHandler<GetProjectBySlugQuery, Result<ProjectDto>>
{
    public async Task<Result<ProjectDto>> Handle(
        GetProjectBySlugQuery query, CancellationToken ct)
    {
