...
public sealed record GetMessagesQuery(int Page = 1, int PageSize = 20) : IRequest<Result<PagedResult<ContactMessageDto>>>;

public sealed class GetMessagesHandler(IContactRepository repo) : IRequestHandler<GetMessagesQuery, Result<PagedResult<ContactMessageDto>>>
{
    public async Task<Result<PagedResult<ContactMessageDto>>> Handle(GetMessagesQuery query, CancellationToken ct)
    {
...
public sealed record GetMessageByIdQuery(string Id) : IRequest<Result<ContactMessageDto>>;

public sealed class GetMessageByIdHandler(IContactRepository repo) : IRequestHandler<GetMessageByIdQuery, Result<ContactMessageDto>>
{
    public async Task<Result<ContactMessageDto>> Handle(GetMessageByIdQuery query, CancellationToken ct)
    {
